...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define MaxCommandsInBuffer 3 //max 10 commands can be received and saved without overwriting. Each command has a max size of APP_RX_DATA_SIZE 
...
static struct
        ...
        unsigned char IsCommandDataReceived;   //any number >0 means data were received. 0 means no data is available 
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)                {
               ...
               /*             
                ...
                if (s_RxBuffers.pos_receive >= MaxCommandsInBuffer)               //reach the last buffer, need to rewind to 0
                        {  s_RxBuffers.pos_receive = 0; }                         //prepare to receive the next data
               ...
               */
                ...
                memcpy( &rx_buf[i], Buf, *Len);
                i = i + *Len;
                if ( rx_buf[i-2] == '\r' &&  rx_buf[i-1]== '\n') 
                { rx_len = i;
                        i = 0; 
